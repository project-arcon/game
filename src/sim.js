  ...
  window.Sim = (function () {
    ...
    Sim.prototype.say = function (message, rootName) {
      if (typeof server !== "undefined" && server !== null) {
        return server.say(message, rootName);
      } else if (this.local) {
    ...
    Sim.prototype.whoIsHost = function () {
      ...
      if (!haveHost) {
        ...
        for (m = 0, len2 = ref1.length; m < len2; m++) {
          p = ref1[m];
          if (!p.ai && p.connected && p.side !== "spectators" && !isSuperUser(p)) {
            p.host = true;
            break;
          }
        }
    ...
    Sim.prototype.addAi = function (player, name, side, aiBuildBar) {
      ...
      if (!this.local) {
        if (this.noAIPlayers) return;
        if (side != "alpha" && side != "beta") return;
        if (this.serverType === "1v1") return;
        if (this.serverType === "1v1r") return;
        if (this.serverType === "1v1t") return;
        if (this.numInTeam(side) >= this.playersPerTeam()) return;
    ...
    Sim.prototype.kickPlayer = function (p, number) {
      ...
      if (player && player.side !== "spectators") {
        ...
        if (player.host && !isSuperUser(player) && isSuperUser(p) && p.side !== "spectator") {
          player.host = false;
          return;
        }
